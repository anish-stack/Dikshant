...
function LMSContent() {
  ...
  const urlVideoParam = urlParams.get("video"); // This is the secureToken from clicked link
      ...
      try {
        ...
        if (status === 401 || status === 403) {
          setError({
            type: "auth",
            message:
              "Access denied or expired. Please check your subscription.",
            icon: Shield,
          });
        } else {
          setError({
...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (urlVideoToken) {
        ...
        if (match) {
          selectedVideo = match;
          console.log("✅ Correct video loaded from URL:", match.title);
        } else {
          console.warn("⚠️ URL token not found in video list, using first video");
        }
      }

      setCurrentVideo(selectedVideo);

      // === SAFE URL SYNC - Only if token is different ===
      ...
      if (currentTokenInUrl !== selectedVideo.secureToken) {
        ...
        // Safe replace without triggering re-render loop
  ...
  fetchData();
}, [isOnline, courseId, token]); // ← Removed urlVideoParam & retryCount if causing loop
// Only dependencies that don't change frequently
// ← Add urlVideoParam as dependency
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background p-4">
        <div className="text-center max-w-2xl">
          <h1 className="text-[150px] lg:text-[200px] font-black text-foreground mb-4 leading-none">
            404
          </h1>
          ...
          <p className="text-muted-foreground text-lg mb-8">
            The page doesn't exist or access denied.
          </p>
          <div className="space-y-3">
            ...
            <button
              ...
              className="px-8 py-3 bg-secondary rounded-lg font-medium ml-3"
            >
              Go Back
            </button>
          </div>
      ...
      <div className="flex h-screen items-center justify-center bg-background p-4">
        <div className="bg-card rounded-2xl shadow-2xl p-8 max-w-md w-full text-center border">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={() => (window.location.href = "/")}
              className="w-full px-6 py-3 bg-secondary rounded-lg"
            >
              Go to Home
            </button>
          </div>
    ...
    <div className="flex h-screen bg-white overflow-hidden text-slate-900">
      ...
      <aside
        className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-40 w-80 bg-white border-r overflow-y-auto transition-transform duration-300`}
      >
        <Sidebar
          videos={videos}
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-slate-50">
        <header
          className={`bg-white border-b px-4 py-3 fixed top-0 left-0 right-0 z-40 transition-transform ${scrolled ? "translate-y-0 shadow-md" : "-translate-y-full"
            } lg:translate-y-0 lg:sticky`}
        >
