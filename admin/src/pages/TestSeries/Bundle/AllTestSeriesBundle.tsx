...
const AllTestSeriesBundle = () => {
  ...
  const fetchBundles = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/testseries-bundles?isAdmin=true`, {
      ...
      Swal.fire("Error", "Failed to load test series bundles", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.delete(`${API_URL}/testseries-bundles/${id}?isAdmin=true`, {
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          ${
            bundle.discountPrice
              ? `<p><strong>Discount Price:</strong> ₹${bundle.discountPrice} 
                 <span class="text-green-600 font-medium">(${Math.round(((bundle.price - bundle.discountPrice) / bundle.price) * 100)}% off)</span></p>`
              : ""
          }
          <p><strong>GST:</strong> ${bundle.gst || 0}%</p>
          <p><strong>Status:</strong> ${bundle.isActive ? '<span class="text-green-600">Active</span>' : '<span class="text-red-600">Inactive</span>'}</p>
          <p><strong>Created:</strong> ${new Date(bundle.createdAt).toLocaleString("en-IN")}</p>
          <hr class="my-4 border-gray-300">
          <h4 class="font-bold text-lg">Included Test Series (${bundle.testSeries?.length || 0})</h4>
            ...
            ${bundle.testSeries
              ?.map(
                (ts) => `
                  <li class="text-sm">
                    ${ts.title} 
                    <span class="text-gray-600"> (₹${ts.price}${ts.discountPrice ? ` → ₹${ts.discountPrice}` : ""})</span>
                  </li>`
              )
              .join("") || "<li class='text-gray-500'>No test series included</li>"}
          </ul>
        </div>
      `,
      width: window.innerWidth < 640 ? "90%" : "700px",
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <h1 className="text-3xl font-bold text-gray-900">Test Series Bundles</h1>
          <button
            onClick={() => navigate("/create-test-series-bundle")}
        ...
        {bundles.length === 0 ? (
          <div className="bg-white rounded-xl shadow p-10 text-center">
            <p className="text-gray-600 text-lg mb-4">No bundles found</p>
            <button
              onClick={() => navigate("/create-test-series-bundle")}
                    ...
                    <tr key={bundle.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <button
                            onClick={() => navigate(`/edit-test-series-bundle/${bundle.id}`)}
...
export default AllTestSeriesBundle;